...
% Engineering steps
prod_rxn = 'r_4780'; % r_4780(heme) r_1589(2-phenylethanol) r_2001 (phenylacetaldehyde) r_2056(succ)  insensitive: r_1761(ethanol) r_1549(2,3-butanediol)
max_iterations = 12;
max_perturbations = 5;
nKOsLimit = 20;
...
for th = 1:length(simulatedThresholds)
    ...
    predicted_vec_log = log(abs(predicted_vec));
    target_exp_valid_log = log(abs(target_exp_valid));

    % Calculate RMSE
    residual = target_exp_valid_log - predicted_vec_log;

    goodIdx = isfinite(residual);
    target_exp_valid_log = target_exp_valid_log(goodIdx);
    predicted_vec_log = predicted_vec_log(goodIdx);

    residual = target_exp_valid_log - predicted_vec_log;
    rmse_value = sqrt(mean(residual.^2));

    % Calculate R²
    ss_res = sum((predicted_vec_log - target_exp_valid_log).^2);
    ss_tot = sum((target_exp_valid_log - mean(target_exp_valid_log)).^2);
    r2 = 1 - (ss_res / ss_tot);
...
% Build the PROM with regulatory constraints (same as engineering)
...
exportFilename = strcat('Sce_strategies_', metName, '.xlsx');
...
exportFilename_sKO = strcat('Sce_strategies_sKO_', metName, '.xlsx');
...
fileName_sKO = strcat('Sce_singleKO_', metName{1,1}, '.xlsx');
...
fileName_loo = strcat('Sce_leaveOneOut_', metName{1,1}, '.xlsx');
