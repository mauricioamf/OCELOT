...
cat("Loading lasso results...\n")
lasso_res_df <- read.csv(file.path(path, "Ecoli_GRN_lasso_raw.csv"), row.names = 1)

# Load filtered expression data
cat("Loading expression data...\n")
expression_data <- read.csv(file.path(path, "Ecoli_count_matrix.csv"), sep = "\t", row.names = 1)
...
TF_TG_GS <- read.delim(file.path(path, "Ecoli_GRN_ecocyc.csv"), header = TRUE, sep = "\t")
TF_TG <- data.frame(
  TF = trimws(TF_TG_GS$TF.locus.tag),
  Target = trimws(TF_TG_GS$TG.locus.tag)
)
...
r2_threshold <- 0.2          # e.g., TFs must explain at least 70% of the variance
correlation_threshold <- 0.2 # e.g., Pearson correlation > 0.5
...
if (length(valid_dfs) > 0) {
  ...
  write.csv(filtered_result_df, file.path(path, "Ecoli_GRN_lasso_values.csv"), row.names = FALSE)
  ...
  if(nrow(filtered_result_df) > 0) {
    ...
    if(ncol(coef_matrix) > 1) {
      ...
      write.csv(coef_matrix_mat, file.path(path, "Ecoli_GRN_lasso.csv"), row.names = TRUE)
      cat("Saved validated wide-format matrix.\n")
    }
  } else {
    cat("No edges passed the strict validation thresholds (R2 >=", r2_threshold, ").\n")
  }
