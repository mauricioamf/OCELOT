function wt = runWTThresholdSelection(modelWT_FBA, biomass_idx, reg, protectedRxnIdx, params)
    ...
    for t = 1:numel(reg.threshold_values)
        ...
        % TF_activity = reg.TF_exp .* state.z_bin;
        % y_tgt = reg.beta_matrix * TF_activity;
        y_rxn = reg.gene_to_rxn_map * y_tgt;
        ...
        y_all_struct.(name) = T;
        z_struct.(name) = z_bin;
        target_expr_struct.(name) = target_exp_hat;
    end
    ...
    for tr = 1:numel(threshold_names)
        ...
        try
            [biomass, gp_WT, result_WT, kappa] = solveProblemWithScaledBounds( ...
                modelWT_FBA, biomass_idx, yWT_valid, reg.gene_to_rxn_map, ...
                protectedRxnIdx, params.mu, rxnIDs, ...
                params.kappaStart, params.kappaMin, params.kappaDecay);
    ...
    for th = 1:numel(simulatedThresholds)
        ...
        % Match original monolithic code exactly
        ...
        % Match original discriminability logic exactly
        ...
        if n_active == 0 || n_active == numel(z_bin_th)
            ...
            y_base = reg.gene_to_rxn_map * (reg.beta_matrix * (reg.TF_exp .* z_bin_th));
